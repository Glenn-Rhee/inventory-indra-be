...
func (r *TransactionRepository) CreateTransaction(data model.CreateTransaction, userId string) (error, int) {
	...
	if data.TransactionType == model.TransactionIN && product.Stock.StockPerButir == 0 {
		tx.Rollback()
		return fmt.Errorf("Stock of %s is empty. Please restock!", product.Name), http.StatusBadRequest
	}
...
func (r *TransactionRepository) GetTransaction(params GetTransactionParams) (model.GetTransaction, error, int) {
	...
	query := r.db.Model(&model.Transaction{}).
		...
		Where("transactions.created_at >= ?", time.Now().AddDate(0, 0, -30)).
		Preload("Product")
